...
if PENDULUM:
    latent_values = [x for x in np.arange(1, 6) for _ in range(5)]
...
for latent in latent_values:
    if PENDULUM:
        ckptno = 225000 if latent <= 4 else 200000
        vild_modelpath = 'logsarchive/pendv2/InvertedPendulumCustomPos{}0/ckpt_policy_T{}.pt'.format(int(latent), ckptno)
    ...
    for i in range(n_episodes):
        ...
        while not done:
            ...
            action += np.random.randn(*action.shape)*0.01 + 0.01
        ...
        r = {
            ...
            'ramp_pos': np.mean([info['ramp_pos'] for info in info_list]),
        }
        rollouts.append(r)
...
for lv, d in zip(latent_values, rollouts):
    sxy = d['observations'] + np.random.randn(*d['observations'].shape)*0.0
    plt.plot(sxy[:, 0], sxy[:, 1]+lv*0.1, 'k', linewidth=1)
    plt.scatter(sxy[:, 0], sxy[:, 1]+lv*0.1, s=4, c='k')
plt.tight_layout()
