...
class Discriminator(nn.Module):
    ...
    ## used for reward. 
    def get_reward(self, s, a=None, ns=None):
...
class Posterior(nn.Module):
    def __init__(self, input_dim, encode_dim, encode_cont=False, hidden_size=(256, 256), activation='relu'):
        super().__init__()
        if activation == 'tanh':
            self.activation = torch.tanh
        elif activation == 'relu':
        ...
        if not self.encode_cont:
            ...
            # for mean and variance, we output two sets of params
            self.mean_head = nn.Linear(last_dim, encode_dim)
            self.logstd_head = nn.Linear(last_dim, encode_dim)

    def forward(self, x):
        for affine in self.affine_layers:
            x = self.activation(affine(x))
        if not self.encode_cont:
            rets = self.score_head(x)
        else:
            rets = self.mean_head(x), self.logstd_head(x)
        return rets
    ...
    def get_logposterior(self, states, actions, latent_code):
        if not self.encode_cont:
            ...
            x = torch.cat((states, actions), 1)
            code_mu, code_logstd_ = self.forward(x)
            code_logstd = torch.clamp(code_logstd_, -20, 2)

            code_var = torch.exp(code_logstd) ** 2
            code_lprob = -((latent_code - code_mu) ** 2) / (2 * code_var) - code_logstd - math.log(math.sqrt(2 * math.pi))
